...
§
[correction] L'agent a déplacé biome.json de .config/language/ vers la racine du dépôt, croyant que biome 2.x ne supportait pas une config nichée (à cause du test 'biome lint' sans argument). C'était une fausse hypothèse : nvim-lint passe le nom de fichier (append_fname), donc une config nichée fonctionne. Une fois la config restaurée, l'agent a résolu le problème résiduel (linter le biome.json lui-même, qui déclenche l'erreur nested root) en configurant nvim-lint pour lancer biomejs depuis le dossier du buffer. <!-- created=2026-08-30, last=2026-08-30 -->
§
[tool-quirk] Biome 2.x `--config-path` rejects '..' in the path and expects a directory, not a file. Renamed configs (e.g. biome.jsonc) are ignored entirely by biome. Also, a config nested in a subdirectory (e.g. .config/language/) triggers 'Found a nested root configuration' when running bare `biome lint` from a parent CWD; however, nvim-lint passes the filename (append_fname), so `biome lint <file>` works fine with a nested config. Any file under the config's dir triggers the quirk when linted from a parent CWD. Solution: keep biome.json nested, and override nvim-lint's biomejs linter to run from the buffer's directory (config discovery walks up from the file). <!-- created=2026-08-30, last=2026-08-30 -->